...
From Lorentz we see that lengths can vary depending on reference frame. If we let $\Delta x = L$ where $L$ is the length of an object we have:
\begin{align}
  L_{0} = \gamma L
\end{align}
Where $L_{0}$ is the rest length, the length of the object in the reference frame in which it is at rest.
...
\begin{align}
  &p^{\mu} = \begin{bmatrix} \gamma m \\ \gamma m \vec{v} \end{bmatrix}
  = \begin{bmatrix} E \\ \vec{p} \end{bmatrix}\\
\end{align}
For an object of mass m. For light with no mass
\begin{align}
  p^{\mu}_{\gamma} = \begin{bmatrix} E \\ E \end{bmatrix}
\end{align}
These vectors really have four components, one momentum component for each direction
and one energy component. But for a one-dimensonal problem we can drop the botton two
components and treat it as a two-dimensonal vector.
...
\begin{align*}
  E^{2} - |p|^{2} &= \gamma^{2} m^{2} - \gamma^{2} m^{2} |v|^{2}\\ \\
  E^{2} - |p|&^{2} = \gamma^{2}m^{2} (1 - |v|^{2})\\
  \\
  \textrm{recall the deffinition of $\gamma$ }\\
